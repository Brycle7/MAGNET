	...
	public class WiFiDevicesAdapter extends ArrayAdapter<WiFiP2pService> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (service != null) {
				...
				if (nameText != null) {
					nameText.setText(service.device.deviceName + ": " +service.device.deviceAddress + " - " + service.instanceName +" - " +" IsGroupOwner: " + service.device.isGroupOwner());
				}
				TextView statusText = (TextView) v
						.findViewById(android.R.id.text2);
				if(service.device.status==WifiP2pDevice.CONNECTED) {
				...
				}else if(service.device.status==WifiP2pDevice.INVITED){
					statusText.setTextColor(Color.YELLOW);
					statusText.setText("Status: " + getDeviceStatus(service.device.status) + " Intention: " + service.getIntention()+ " Pass: " + service.PassPhrase);
				}else if(service.device.status==WifiP2pDevice.AVAILABLE){
